...
const ResponseFilter = () => {
  ...
  return (
    <Popover open={isOpen} onOpenChange={handleOpenChange}>
      <PopoverTrigger className="flex items-center justify-between rounded border border-slate-200 bg-white p-3 text-sm text-slate-600 hover:border-slate-300 sm:px-6 sm:py-3 mt-[72.9453px] mb-[0.0390625px]">
        <div className="text-[#FDFCFCFF]" >Filter {filterValue.filter.length > 0 && `(${filterValue.filter.length})`}</div>
        <div className="ml-3">
          {isOpen ? (
